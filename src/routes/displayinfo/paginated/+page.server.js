...
import getSupaInfo from '$lib/supabaseInfo.js';
import getPagination from '$lib/supabasePagination';
import getPaginationInfo from '$lib/supabasePaginationInfo.js';
...
export async function load({ url, params, route }) {
    ...
    const data = await getPaginationInfo(page, amount);

    const tableData = await getSupaInfo();
